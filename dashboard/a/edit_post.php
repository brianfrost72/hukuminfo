...
// PROTEKSI EDIT POST DARI USER LAIN
$post_id = (int)$_GET['id'];
...
if (!$qPost || mysqli_num_rows($qPost) == 0) {
    die('Postingan tidak ditemukan');
}

$post = mysqli_fetch_assoc($qPost);

$qAuthor = mysqli_query($conn, "
    SELECT full_name
    FROM user_profile
...
$author = mysqli_fetch_assoc($qAuthor);
...
|--------------------------------------------------------------------------
*/
if (isset($_POST['update_postingan'])) {
    ...
    */
    ...
    if (!empty($_FILES['post_image']['name'])) {
        ...
        if (
            ...
            file_exists('../assets/images/uploads/posts/' . $post['post_image'])
        ) {
            unlink('../assets/images/uploads/posts/' . $post['post_image']);
        }
...
UPDATE post SET
    ...
    post_subcategory_id='$post_subcategory_id',
        ...
        if (!empty($_POST['tags'])) {
            ...
            if (is_array($tags)) {
                ...
                foreach ($tags as $tag) {
                    ...
                    $tagNameEsc = mysqli_real_escape_string($conn, $tagName);

                    $cekTag = mysqli_query($conn, "
                    SELECT id
                    FROM tags
                    WHERE tag_name='$tagNameEsc'
                    LIMIT 1
                ");

                    if (mysqli_num_rows($cekTag)) {

                        $tagData = mysqli_fetch_assoc($cekTag);
                        $tagId = $tagData['id'];
                    } else {

                        $tagSlug = strtolower($tagName);
                        $tagSlug = preg_replace('/[^a-z0-9]+/', '-', $tagSlug);
...
while ($tag = mysqli_fetch_assoc($qPostTags)) {
    $postTags[] = [
        'value' => $tag['tag_name']
    ];
}
...
<head>
    ...
    <title>Edit Postingan - Dashboard | Hukuminfo.id</title>
                    ...
                    <div class="d-flex align-items-center justify-content-between mb-4">
                        <div>
                            ...
                            <p class="text-muted mb-0">
                                Lengkapi informasi untuk mengedit artikel atau berita.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="card p-4" style="border-radius:14px; border:none;">
                        <form method="POST"
                            ...
                            <div class="form-group mb-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <input
                                        ...
                                        name="post_sub_title"
                                        class="form-control"
                                        value="<?= htmlspecialchars($post['post_sub_title']); ?>">
                                </div>
                            </div>
                                            ...
                                            <div class="flex-fill">
                                                <select
                                                    ...
                                                    <?php while ($k = mysqli_fetch_assoc($kategori)) : ?>
                                                        <option value="<?= $k['id']; ?>"
                                                            <?= ($k['id'] == $post['post_category_id']) ? 'selected' : ''; ?>>
                                            ...
                                            <div class="flex-fill">
                                                <select
                                                    ...
                                                    <?php while ($s = mysqli_fetch_assoc($subkategori)) : ?>
                                                        <option value="<?= $s['id']; ?>"
                                                            <?= ($s['id'] == $post['post_subcategory_id']) ? 'selected' : ''; ?>>
                            ...
                            <div class="form-group mb-4">
                                ...
                                <textarea name="post_desc" id="summernote"><?= htmlspecialchars($post['post_desc']); ?></textarea>
                            </div>
        ...
        ">
                    ...
                    ">
                        ...
                        ">
                                    ...
                                    <div
                                        id="previewContainer"
                                        class="mt-4"
                                        style="<?= !empty($post['post_image']) ? '' : 'display:none;' ?>">

                                        <div class="d-flex align-items-center">

                                            <img
                                                id="previewImage"
                                                src="<?= !empty($post['post_image']) ? '../assets/images/uploads/posts/' . $post['post_image'] : '' ?>"
                                                alt="Preview"
                                                style="
                        width:180px;
                        height:120px;
                    ...
                    ">
                                            ...
                                            <div class="ml-3">
                                                ...
                                                <small
                                                    id="fileName"
                                                    class="text-muted"><?= htmlspecialchars($post['post_image']); ?>
                                                </small>
...
========================= -->
    ...
    <script>
        $('#post_category_id').on('change', function() {
            ...
            $(document).ready(function() {
                ...
                $('#post_category_id').trigger('change');

                setTimeout(function() {

                    $('#post_subcategory_id')
                        .val('<?= $post['post_subcategory_id']; ?>')
                        .trigger('change');
    ...
    <script>
        ...
        document.querySelector('form').addEventListener('submit', function() {
            ...
            document.getElementById('tagsJson').value =
                JSON.stringify(tagify.value);
